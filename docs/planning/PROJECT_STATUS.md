...
Release 1.9.0 is the current local release candidate. It upgrades Universal Search, dynamic in-app notifications, shared empty/loading/error infrastructure, accessibility, application recovery, and initial-load performance while preserving the compact Home daily brief. The initial production JavaScript bundle is approximately 25 KB smaller through lazy loading. No module, database, migration, dependency, authentication, permission, or business-logic change is included.
...
Next product scope after 1.9.0: separately plan and number Gardening Operations without mixing it into the platform-quality release.
