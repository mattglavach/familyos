...
FamilyOS is a mobile-first React app for household operations: calendar overview, tasks, pool care, college planning, and finance snapshots. The current Phase 1 app is a Vite frontend with Vercel serverless API support and Supabase REST persistence.
...
5. Optional: set `VITE_APPROVED_HOUSEHOLD_EMAILS` in `.env.local` and Vercel to a comma-separated list of the two approved household emails. This is a browser-visible UX allowlist for friendlier errors; Supabase manually-created users and row-level security remain the real access control.
...
| --- | --- | --- |
| `VITE_SUPABASE_URL` | Browser | Supabase project REST URL. |
| `VITE_SUPABASE_ANON_KEY` | Browser | Supabase anon key. Configure RLS appropriately. |
| `VITE_APPROVED_HOUSEHOLD_EMAILS` | Browser | Optional comma-separated household email allowlist for friendlier login errors. |
| `VITE_GOOGLE_CLIENT_ID` | Browser | Google OAuth web client ID. |
| `VITE_GOOGLE_CALENDAR_ID` | Browser | Calendar ID to read, often `primary` or an email address. |
...
During the Vite migration window, legacy `REACT_APP_*` browser variables are still accepted as a fallback, but new configuration should use `VITE_*`.
...
- `VITE_APPROVED_HOUSEHOLD_EMAILS` if using the optional household email allowlist
...
Supabase Auth does not use a `VITE_SITE_URL` value in this app. Password sign-in is the default login path. Magic-link redirects are derived from `window.location.origin`, and the matching production and preview origins must be present in Supabase Auth URL configuration for fallback magic links.
