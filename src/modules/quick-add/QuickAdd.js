...
export function QuickAdd({onNavigate, openSignal = 0, initialMode = null}){
  ...
  async function saveHabit(){if(submitting)return;if(!form.title?.trim()){setSaveError("Name is required");return;}const actions=(form.actions||"").split("\n").map(x=>x.trim()).filter(Boolean);if(form.habit_type==="checklist"&&!actions.length){setSaveError("Add at least one action.");return;}try{setSubmitting(true);const habit=await habits.insert({name:form.title.trim(),description:form.notes||"",frequency:form.frequency==="weekday"?"daily":form.frequency||"daily",active_days:form.frequency==="weekday"?[1,2,3,4,5]:[],...(Number(form.target_count)>1?{target_count:Number(form.target_count)}:{}),habit_type:form.habit_type||"simple",completion_threshold_mode:form.threshold_mode||"all",completion_threshold_count:form.threshold_mode==="count"?Math.max(1,Number(form.threshold_count)||1):null,visibility:form.visibility||"household",important:Boolean(form.important),start_date:TODAY_STR,status:"active",archived:false});for(let index=0;index<actions.length;index+=1)await habitActions.insert({habit_id:habit.id,name:actions[index],display_order:index,required:true,active:true});close();setToast({message:"Habit created."});onNavigate("habits");}catch(e){setSaveError(formatUserFacingError(e,"Habit could not be saved right now."));}finally{setSubmitting(false);}}
  ...
  return(<>
    ...
    <OriginDrawer open={open} onOpenChange={(nextOpen)=>{ if(!nextOpen) close(); }} title={modeTitle}>
        ...
        {mode==="habit"&&<FormSection><FormGroup><Label htmlFor="quick-habit-name">Name</Label><Input id="quick-habit-name" autoFocus value={form.title||""} onChange={e=>setField("title",e.target.value)} placeholder="Read, exercise, feed pets..."/></FormGroup><FormGroup><Label>Type</Label><SegmentedControl value={form.habit_type||"simple"} options={[{value:"simple",label:"Simple"},{value:"checklist",label:"Checklist"}]} ariaLabel="Habit type" onValueChange={value=>setField("habit_type",value)}/></FormGroup>{form.habit_type==="checklist"&&<><NotesField id="quick-habit-actions" label="Actions" placeholder="One action per line" value={form.actions||""} onChange={value=>setField("actions",value)}/><FormGroup><Label>Completion threshold</Label><SegmentedControl value={form.threshold_mode||"all"} options={[{value:"any",label:"Any one"},{value:"count",label:"Number"},{value:"all",label:"All"}]} ariaLabel="Completion threshold" onValueChange={value=>setField("threshold_mode",value)}/></FormGroup>{form.threshold_mode==="count"&&<FormGroup><Label>Actions required</Label><Input type="number" min="1" value={form.threshold_count||""} onChange={e=>setField("threshold_count",e.target.value)}/></FormGroup>}</>}<FormGroup><Label>Frequency</Label><SegmentedControl value={form.frequency||"daily"} options={[{value:"daily",label:"Daily"},{value:"weekday",label:"Weekdays"},{value:"weekly",label:"Weekly"},{value:"monthly",label:"Monthly"}]} ariaLabel="Habit frequency" onValueChange={value=>setField("frequency",value)}/></FormGroup>{["weekly","monthly"].includes(form.frequency)&&<FormGroup><Label htmlFor="quick-habit-target">Completion goal</Label><Input id="quick-habit-target" type="number" min="1" max={form.frequency==="weekly"?7:31} value={form.target_count||""} onChange={e=>setField("target_count",e.target.value)}/></FormGroup>}<FormGroup><Label>Visibility</Label><SegmentedControl value={form.visibility||"household"} options={[{value:"household",label:"Household"},{value:"personal",label:"Personal"}]} ariaLabel="Habit visibility" onValueChange={value=>setField("visibility",value)}/></FormGroup><NotesField id="quick-habit-notes" value={form.notes||""} onChange={value=>setField("notes",value)}/><ValidationSummary error={saveError}/><SaveCancelFooter saveLabel="Add Habit" cancelLabel="Back" onSave={saveHabit} onCancel={()=>setMode(null)} submitting={submitting}/></FormSection>}
