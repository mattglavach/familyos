...
import { CalendarDays, Check, ChevronLeft, ClipboardList, Droplets, ListChecks, NotebookPen, Repeat2, Settings2, Sparkles, Utensils } from "lucide-react";
...
export function QuickAdd({onNavigate, openSignal = 0, initialMode = null}){
  ...
  const habitActions = useTable("habit_actions", "display_order", true, { userId: null });
  ...
  function close(){setOpen(false);setMode(null);setForm({});setSmartText("");setParsed(null);setSaveError(null);setSubmitting(false);}
  ...
  const options=[
    ...
    {id:"pool-treatment", icon:Droplets, label:"Pool Activity", status:"Ready", featured:true, enabled:true, accentClass:"border-l-primary", iconClass:"text-primary"},
  ...
  function analyzeSmart(){setParsed(parseQuickAdd(smartText,{timezone:household.householdSettings?.timezone,members:household.people}));}
  function confirmSmart(){if(!parsed?.valid)return;const mapped=parsed.module==="garden"?"task":parsed.module;setForm({title:parsed.title,due_date:parsed.date,date:parsed.date,time:parsed.time,notes:parsed.notes,assigned_person_id:parsed.memberId,...parsed.measurements});setMode(mapped);setParsed(null);}
  ...
  async function saveHabit(){if(submitting)return;if(!form.title?.trim()){setSaveError("Name is required");return;}const actions=(form.actions||"").split("\n").map(x=>x.trim()).filter(Boolean);if(form.habit_type==="checklist"&&!actions.length){setSaveError("Add at least one routine step.");return;}try{setSubmitting(true);const habit=await habits.insert({name:form.title.trim(),description:form.notes||"",category:form.habit_category||"Other",frequency:form.frequency==="weekday"?"daily":form.frequency||"daily",active_days:form.frequency==="weekday"?[1,2,3,4,5]:[],...(Number(form.target_count)>1?{target_count:Number(form.target_count)}:{}),habit_type:form.habit_type||"simple",completion_threshold_mode:form.threshold_mode||"all",completion_threshold_count:form.threshold_mode==="count"?Math.max(1,Number(form.threshold_count)||1):null,visibility:form.visibility||"household",important:Boolean(form.important),start_date:TODAY_STR,status:"active",archived:false});for(let index=0;index<actions.length;index+=1)await habitActions.insert({habit_id:habit.id,name:actions[index],description:"",display_order:index,required:true,active:true});close();setToast({message:form.habit_type==="checklist"?"Routine created.":"Habit created."});onNavigate({tab:"habits",habitId:habit.id});}catch(e){setSaveError(formatUserFacingError(e,"Habit could not be saved right now."));}finally{setSubmitting(false);}}
  ...
  return(<>
    ...
    <OriginDrawer open={open} onOpenChange={(nextOpen)=>{ if(!nextOpen) close(); }} restoreFocus={quickAddTrigger} title={modeTitle}>
        ...
        {mode==="habit"&&<FormSection><FormGroup><Label htmlFor="quick-habit-name">Name</Label><Input id="quick-habit-name" autoFocus value={form.title||""} onChange={e=>setField("title",e.target.value)} placeholder="Read, exercise, morning routine..."/></FormGroup><FormGroup><Label>Type</Label><SegmentedControl value={form.habit_type||"simple"} options={[{value:"simple",label:"Simple habit"},{value:"checklist",label:"Routine"}]} ariaLabel="Habit or routine type" onValueChange={value=>setField("habit_type",value)}/></FormGroup>{form.habit_type==="checklist"&&<><NotesField id="quick-habit-actions" label="Routine steps" placeholder="One step per line" value={form.actions||""} onChange={value=>setField("actions",value)}/><FormGroup><Label>Completion rule</Label><SegmentedControl value={form.threshold_mode||"all"} options={[{value:"any",label:"Any one"},{value:"count",label:"Number"},{value:"all",label:"All required"}]} ariaLabel="Completion rule" onValueChange={value=>setField("threshold_mode",value)}/></FormGroup>{form.threshold_mode==="count"&&<FormGroup><Label>Steps required</Label><Input type="number" min="1" value={form.threshold_count||""} onChange={e=>setField("threshold_count",e.target.value)}/></FormGroup>}</>}<FormGroup><Label htmlFor="quick-habit-category">Category</Label><select id="quick-habit-category" className="flex h-11 w-full rounded-md border border-input bg-secondary px-3 py-2 text-sm text-foreground" value={form.habit_category||"Other"} onChange={e=>setField("habit_category",e.target.value)}>{HABIT_CATEGORIES.map(category=><option key={category}>{category}</option>)}</select></FormGroup><FormGroup><Label>Frequency</Label><SegmentedControl value={form.frequency||"daily"} options={[{value:"daily",label:"Daily"},{value:"weekday",label:"Weekdays"},{value:"weekly",label:"Weekly"},{value:"monthly",label:"Monthly"}]} ariaLabel="Habit frequency" onValueChange={value=>setField("frequency",value)}/></FormGroup>{["weekly","monthly"].includes(form.frequency)&&<FormGroup><Label htmlFor="quick-habit-target">Completion goal</Label><Input id="quick-habit-target" type="number" min="1" max={form.frequency==="weekly"?7:31} value={form.target_count||""} onChange={e=>setField("target_count",e.target.value)}/></FormGroup>}<FormGroup><Label>Visibility</Label><SegmentedControl value={form.visibility||"household"} options={[{value:"household",label:"Household"},{value:"personal",label:"Personal"}]} ariaLabel="Habit visibility" onValueChange={value=>setField("visibility",value)}/></FormGroup><NotesField id="quick-habit-notes" value={form.notes||""} onChange={value=>setField("notes",value)}/><ValidationSummary error={saveError}/><SaveCancelFooter saveLabel={form.habit_type==="checklist"?"Add Routine":"Add Habit"} cancelLabel="Back" onSave={saveHabit} onCancel={()=>setMode(null)} submitting={submitting}/></FormSection>}
