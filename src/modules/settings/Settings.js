  ...
  return (
    <div style={S.screen} className="space-y-5">
      ...
      <Card>
        ...
        <CardContent className="space-y-2 px-4 pb-4 pt-0">{[["enabled","Enable AI Workspace"],["promptPreview","Prompt preview"],["copyBeforeSend","Copy before send"],["privacyFiltering","Privacy filtering"],["hideSensitiveFinancialData","Hide sensitive financial data"],["hideChildInformation","Hide child information"],["homeAiBriefVisible","Show AI Brief on Home"]].map(([key,label])=><div key={key} className="flex min-h-11 items-center justify-between gap-3 border-b border-border py-2"><Label htmlFor={`ai-${key}`}>{label}</Label><Switch id={`ai-${key}`} checked={Boolean(aiSettings[key])} onCheckedChange={value=>setAiSettings(previous=>({...previous,[key]:value}))}/></div>)}<FormGroup><Label htmlFor="ai-provider">Default provider</Label><Select id="ai-provider" value={aiSettings.defaultProvider} onChange={event=>setAiSettings(previous=>({...previous,defaultProvider:event.target.value}))}><option>ChatGPT</option><option>Claude</option><option>Other</option></Select></FormGroup><FormGroup><Label htmlFor="ai-default-prompt">Default prompt template</Label><Select id="ai-default-prompt" value={aiSettings.defaultPromptTemplate} onChange={event=>setAiSettings(previous=>({...previous,defaultPromptTemplate:event.target.value}))}><option>What needs attention this week?</option><option>What should I accomplish today?</option><option>Prepare my weekend.</option><option>Summarize my family's schedule.</option></Select></FormGroup><FormGroup><Label htmlFor="home-card-order">Home card order</Label><Select id="home-card-order" value={(aiSettings.homeCardOrder||[]).join(",")} onChange={event=>setAiSettings(previous=>({...previous,homeCardOrder:event.target.value.split(",")}))}><option value="priorities,schedule,snapshot">Priorities, Schedule, Snapshot</option><option value="schedule,priorities,snapshot">Schedule, Priorities, Snapshot</option><option value="priorities,snapshot,schedule">Priorities, Snapshot, Schedule</option></Select></FormGroup><Button type="button" className="w-full" onClick={()=>{writeAiSettings(aiSettings);refreshSnapshot();notify("AI and Home settings saved on this device.");}}><Save className="h-4 w-4"/>Save AI & Home Settings</Button></CardContent>
      </Card>
