...
export function Settings({ auth, gc, secureCalendar }) {
  ...
  return (
    <div style={S.screen} className="space-y-5">
      ...
      <Card>
        <CardHeader className="p-4 pb-2"><CardTitle className="flex items-center gap-2 text-base"><ShieldCheck className="h-4 w-4 text-primary"/>AI Settings</CardTitle><CardDescription>Control Family Assistant context and frequency. Provider credentials and technical prompts remain server-side.</CardDescription></CardHeader>
        <CardContent className="space-y-2 px-4 pb-4 pt-0">{[["enabled","Enable AI features"],["familyBriefEnabled","Enable AI Family Brief"],["hideSensitiveFinancialData","Exclude sensitive financial data"],["hideChildInformation","Exclude child information"]].map(([key,label])=><div key={key} className="flex min-h-11 items-center justify-between gap-3 border-b border-border py-2"><Label htmlFor={`ai-${key}`}>{label}</Label><Switch id={`ai-${key}`} checked={Boolean(aiSettings[key])} onCheckedChange={value=>setAiSettings(previous=>({...previous,[key]:value}))}/></div>)}<FormGroup><Label htmlFor="ai-response-length">Preferred response length</Label><Select id="ai-response-length" value={aiSettings.responseLength||"concise"} onChange={event=>setAiSettings(previous=>({...previous,responseLength:event.target.value}))}><option value="concise">Concise</option><option value="standard">Standard</option><option value="detailed">Detailed</option></Select></FormGroup><FormGroup><Label htmlFor="ai-frequency">Recommendation frequency</Label><Select id="ai-frequency" value={aiSettings.recommendationFrequency||"normal"} onChange={event=>setAiSettings(previous=>({...previous,recommendationFrequency:event.target.value}))}><option value="minimal">Minimal</option><option value="normal">Normal</option><option value="frequent">Frequent</option></Select></FormGroup><div className="rounded-lg border border-border bg-secondary/30 p-3 text-xs leading-5 text-muted-foreground"><b className="text-foreground">Privacy</b><br/>FamilyOS sends only selected, permission-scoped context. Attachments, credentials, tokens, database metadata, and full prompts are not stored. Household text is treated as untrusted data and cannot override system rules.</div><Button type="button" className="w-full" onClick={()=>{writeAiSettings(aiSettings);refreshSnapshot();notify("AI settings saved on this device.");}}><Save className="h-4 w-4"/>Save AI Settings</Button><Button type="button" variant="secondary" className="w-full" onClick={async()=>{if(household.householdId&&auth.session?.user?.id)await supabase.from("ai_feedback").delete().eq("household_id",household.householdId).eq("user_id",auth.session.user.id);notify("Recommendation preferences reset where available.");}}>Reset recommendation preferences</Button></CardContent>
      </Card>
