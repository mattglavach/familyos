...
export function Dashboard({ onNavigate, gc, secureCalendar, deps }) {
  ...
  const homeSections = {
    priorities: <section key="priorities"><SectionHeader title="Top Priorities" count={priorities.length} tone="amber" />{priorities.length ? <Card><CardContent className="px-4 py-1">{priorities.map(task => <button key={task.id} type="button" onClick={() => onNavigate({ tab: "tasks", search: task.title || "" })} className="flex min-h-11 w-full items-center gap-3 border-b border-border py-1.5 text-left last:border-0"><ListTodo className="h-4 w-4 shrink-0 text-amber-300"/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{task.title}</span><span className="block text-xs text-muted-foreground">{dateLabel(task.due_date, TODAY_STR)}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground"/></button>)}</CardContent></Card> : <Card><CardContent className="p-4 text-sm text-muted-foreground">You are clear for today. New due, overdue, or high-priority tasks will appear here.</CardContent></Card>}</section>,
    schedule: <section key="schedule"><SectionHeader title="Today's Schedule" count={allScheduleCount} tone="blue" action={allScheduleCount > 3 ? <Button type="button" variant="ghost" size="xs" onClick={() => onNavigate("calendar")}>View All</Button> : null} />{schedule.length ? <Card><CardContent className="px-4 py-1">{schedule.map(event => <button key={event.id || `${event.date}-${event.title}`} type="button" onClick={() => onNavigate({ tab: "calendar", eventId: event.id })} className="flex min-h-11 w-full items-center gap-3 border-b border-border py-1.5 text-left last:border-0"><CalendarDays className="h-4 w-4 shrink-0 text-primary" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{event.title || event.summary || "Calendar event"}</span><span className="block truncate text-xs text-muted-foreground">{formatCalendarEventTime(event)}{event.location ? ` · ${event.location}` : ""}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" /></button>)}</CardContent></Card> : <Card><CardContent className="p-4 text-sm text-muted-foreground">Nothing is scheduled today. Connected Calendar events will appear here automatically.</CardContent></Card>}</section>,
    snapshot: <section key="snapshot"><SectionHeader title="Family Snapshot" tone="neutral" />{familySnapshot.length ? <Card><CardContent className="px-4 py-1">{familySnapshot.map(row => { const Icon = row.icon; return <div key={row.label} className="border-b border-border py-1.5 last:border-0"><button type="button" onClick={() => onNavigate(row.tab)} className="flex min-h-11 w-full items-center gap-3 text-left"><Icon className="h-4 w-4 shrink-0 text-primary" aria-hidden="true" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{row.label}</span><span className="block truncate text-xs text-muted-foreground">{row.detail}</span>{row.details?.slice(0,2).map(detail=><span key={detail} className="mt-0.5 block truncate text-xs text-amber-300">• {detail}</span>)}</span><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" /></button></div>; })}</CardContent></Card> : <Card><CardContent className="p-4 text-sm text-muted-foreground">Add tasks, lists, Pool records, or planning details to build your household snapshot.</CardContent></Card>}</section>,
  };
  return <div style={S.screen} className="space-y-3 overflow-x-hidden">
    ...
    {aiSettings.enabled&&aiSettings.homeAiBriefVisible&&<Card><CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><Bot className="h-5 w-5 shrink-0 text-primary"/><div className="min-w-0 flex-1"><div className="text-sm font-bold">AI Brief</div><div className="break-words text-xs text-muted-foreground">{lastPrompt?`Last prompt: ${lastPrompt.question}`:`Try: ${aiSettings.defaultPromptTemplate}`}</div></div><Button type="button" size="xs" variant="secondary" onClick={()=>onNavigate("ai-workspace")}>Open AI Workspace</Button></CardContent></Card>}
    {(aiSettings.homeCardOrder || ["priorities","schedule","snapshot"]).map(key => homeSections[key]).filter(Boolean)}
  </div>;
}
