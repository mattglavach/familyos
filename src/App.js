...
function useGoogleCalendar() {
  ...
  function signIn() {
    if (!window.google) { setError("Google script not loaded yet — try again in a moment."); return; }
  ...
  const fetchEvents = useCallback(async (accessToken) => {
    ...
    try {
      ...
      const mapped = (data.items || []).map((e, i) => {
        ...
        const member    = assignMember(e.summary, e.description);
        return { id: e.id || i, title: e.summary || "(No title)", date: dateStr, time: timeStr, allDay, member, location: e.location || "" };
      });
      setEvents(mapped);
    } catch (err) {
...
function daysBetween(dateStr) {
  ...
  return Math.round((d - todayReal) / (1000*60*60*24));
}
...
function useTable(table, orderCol, orderAsc=false) {
  ...
  async function insert(row) {
    try {
      ...
      if (!error) await load();
      else setData(prev=>[{...row,id:String(Date.now())},...(prev||[])]);
      return newRow;
    } catch { setData(prev=>[{...row,id:String(Date.now())},...(prev||[])]); }
  }
  async function update(id,row) {
    try {
      await sb.from(table).eq("id",id).update(row);
      await load();
    } catch { setData(prev=>prev.map(r=>r.id===id?{...r,...row}:r)); }
  }
...
function CalendarBanner({ gc }) {
  ...
  return (
    <div style={S.gcBanner}>
      ...
      <div style={{ fontSize:12, color:COLORS.slateLight, marginBottom:12, lineHeight:1.5 }}>
        Sign in with Google to load your real family events automatically.
      </div>
  ...
  return (
    <div style={S.screen}>
      <div style={{ ...S.card, background:COLORS.navyLight, borderColor:COLORS.blue+"44" }}>
        <div style={{ fontSize:11, color:COLORS.blue, fontWeight:700, letterSpacing:"0.8px", textTransform:"uppercase", marginBottom:6 }}>
        ...
        <div style={{ fontSize:13, color:COLORS.slate, marginTop:4 }}>
          {thisWeek.length} events this week · {urgentDeadlines.length} deadline{urgentDeadlines.length!==1?"s":""} approaching
        </div>
      </div>
      ...
      {!gc.token && thisWeek.length===0 && (
        <div style={{ ...S.empty, marginTop:8 }}>Connect Google Calendar above to see your family's events here.</div>
      )}
    ...
    <div style={S.screen}>
      ...
      {gc.token && <>
        ...
        {!gc.loading && days.map(day=>{
          ...
          return (
            <div key={day}>
              <div style={{ ...S.sectionLabel, color:isToday?COLORS.blue:COLORS.slate }}>
        ...
        {!gc.loading && filtered.length===0 && (
          <div style={S.empty}>No events found for the next 30 days.</div>
        )}
      </>}
    </div>
  );
    ...
    <div style={S.screen}>
      ...
      {tab==="deadlines" && <>
        {deadlines.loading?<Loading/>:<>
          ...
          {done.length>0&&<><div style={{ ...S.sectionLabel,marginTop:20 }}>Completed</div>
            {done.map(d=>(
      ...
      {showModal==="deadline"&&<Modal title="Add Deadline" onClose={()=>setShowModal(null)}>
      ...
      {showModal==="school"&&<Modal title="Add School" onClose={()=>setShowModal(null)}>
      ...
      {showModal==="score"&&<Modal title="Log SAT Score" onClose={()=>setShowModal(null)}>
...
function HomeMgmt() {
  ...
  return (
    <div style={S.screen}>
      ...
      {showModal&&<Modal title="Add Maintenance Item" onClose={()=>setShowModal(false)}>
...
function Pool() {
  ...
  async function saveReading() {
    await readings.insert({ date:form.date||TODAY_STR, ph:+form.ph, free_chlorine:+form.free_chlorine, salt:+form.salt, cya:+form.cya, alkalinity:+form.alkalinity, water_temp:+form.water_temp||null, swg_setting:+form.swg_setting||null, notes:form.notes||"" });
  ...
  return (
    <div style={S.screen}>
      ...
      {showLog&&<Modal title="Log Pool Reading" onClose={()=>setShowLog(false)}>
        <label style={S.label}>Date</label>
        <input type="date" style={S.input} value={form.date||""} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/>
        <div style={S.row}>
      ...
      {showMaint&&<Modal title="Log Pool Maintenance" onClose={()=>setShowMaint(false)}>
        <label style={S.label}>Date</label>
        <input type="date" style={S.input} value={form.date||""} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/>
