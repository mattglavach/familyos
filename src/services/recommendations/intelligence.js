...
export function confidenceFor(item,history=[],now=new Date()){
  ...
  const completed=rows.filter(x=>x.action==="completed").length,dismissed=rows.filter(x=>["dismissed","never_remind"].includes(x.action)).length,notUseful=rows.filter(x=>x.action==="not_helpful").length,ignored=Math.max(0,rows.filter(x=>x.action==="generated").length-completed-dismissed);
  const factors=[{name:"deterministic evidence",value:(item.supportingData||[]).length?18:6},{name:"urgency",value:item.urgency==="immediate"?18:item.urgency==="soon"?10:4},{name:"household impact",value:["critical","high"].includes(item.severity)?16:8},{name:"cross-module corroboration",value:(item.sourceModules||[]).length>1?14:4},{name:"freshness",value:date(item.lastEvaluation)>now.getTime()-DAY?12:5},{name:"completion history",value:Math.min(12,completed*4)},{name:"dismissal history",value:-Math.min(24,dismissed*8+ignored*3)},{name:"bounded negative feedback",value:-Math.min(12,notUseful*4)}];
...
export function enhanceRecommendations(items=[],{data={},history=[],today,settings={}}={}){
  const workload=estimateWorkload(data,today),now=new Date();let enhanced=resolveDependencies(items).map(item=>{const confidence=confidenceFor(item,history,now),rows=history.filter(x=>x.recommendation_key===keyOf(item)),lastGenerated=[...rows].filter(x=>x.action==="generated").sort((a,b)=>date(b.created_at)-date(a.created_at))[0],fatigueSignal=rows.some(x=>["dismissed","snoozed","not_helpful","ignored"].includes(x.action)),materialChange=lastGenerated&&lastGenerated.trigger_signature!==item.triggerSignature,cooldownDays=adaptiveCooldownDays(item,history),cooling=fatigueSignal&&lastGenerated&&!materialChange&&date(lastGenerated.created_at)+cooldownDays*DAY>now.getTime();return{...item,confidenceScore:confidence.score,confidenceReason:confidence.reason,confidenceFactors:confidence.factors,cooldownDays,materialChange:Boolean(materialChange),suppressedByFatigue:Boolean(cooling),priorityScore:(item.priorityScore||0)+confidence.score/10-(item.blocked?20:0)};}).filter(x=>!x.suppressedByFatigue||x.materialChange);
  enhanced=groupRecommendations(enhanced).sort((a,b)=>b.priorityScore-a.priorityScore||b.confidenceScore-a.confidenceScore);
  enhanced=enhanced.map((item,index)=>({...item,rank:index+1,whyNow:item.dueTiming&&item.dueTiming!=="Flexible"?`${item.dueTiming}: ${item.rationale}`:item.rationale,whyItMatters:item.consequenceIfIgnored,whyRanked:`Ranked #${index+1} using urgency, impact, confidence, workload, dependencies, and fatigue.`,canWaitReason:item.blocked?item.dependencyReason:item.dueTiming&&!["Today","Tomorrow"].includes(item.dueTiming)?`Due ${item.dueTiming}; no earlier safety or deadline trigger was detected.`:"No current deadline or safety trigger was detected."}));
