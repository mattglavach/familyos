...
export function normalizeTimeline(input={},options={}){
  ...
  const rows=[
    ...
    ...scoped(input.activity).map(row=>item(row.entity_type||"activity",row,{title:row.title||"Household activity",summary:row.summary||"",timestamp:row.occurred_at,effectiveDate:row.occurred_at,status:row.action||"recorded",deepLink:row.deep_link||"timeline",isCompleted:row.action==="completed"})),
  ];
  const seen=new Set();
