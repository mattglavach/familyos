...
function Read-ApprovedMigrationManifest {
  ...
  if ($manifest.expectedLatestRelease -ne '3.2.0') { throw 'The approved manifest must target Release 3.2.0.' }
  if ($manifest.expectedMigrationCount -ne 25) { throw 'The production migration-history count must be exactly 25.' }
  if ($manifest.expectedExecutionMigrationCount -ne 23 -or $manifest.approvedMigrations.Count -ne 23) { throw 'The executable migration count must be exactly 23.' }
  ...
  if (($versions | Select-Object -Unique).Count -ne 25) { throw 'Production migration versions must be unique.' }
  ...
  if ($manifest.approvedMigrations[-1].release -ne '3.2.0' -or $manifest.approvedMigrations[-1].version -ne '20260714030000' -or $manifest.approvedMigrations[-1].file -ne $manifest.expectedLatestMigrationFile) { throw 'The latest approved migration does not align with Release 3.2.0.' }
