...
Assert-True ($manifest.expectedMigrationCount -eq 25 -and $manifest.approvedMigrations.Count -eq 23 -and $manifest.historyOnlyMigrations.Count -eq 2) 'production and execution migration counts reconcile'
Assert-True ($manifest.expectedLatestRelease -eq '3.2.0' -and $manifest.approvedMigrations[-1].version -eq '20260714030000' -and $manifest.approvedMigrations[-1].file -eq $manifest.expectedLatestMigrationFile) 'latest migration aligns to 3.2.0'
...
Assert-True (($allVersions | Select-Object -Unique).Count -eq 25 -and ($allFiles | Select-Object -Unique).Count -eq 25) 'no migration version or file appears twice'
...
try {
  ...
  Assert-True ($output -match '(?m)^Approved: .*20260714030000_release_3_2_ai_planning.sql') 'dry run reaches 3.2.0'
