...
test("Release 2.3 Home, navigation, task views, Habits, and Routines", async ({ page }) => {
  ...
  expect(await page.locator("section").filter({ hasText: /Today.s Focus/ }).getByRole("button").count()).toBeLessThanOrEqual(3);
  const nav = page.getByRole("navigation", { name: "Primary navigation" });
  await expect(nav.getByRole("button", { name: "Attention", exact: true })).toBeVisible();
  ...
  await page.getByRole("button", { name: `Complete ${marker} habit`, exact: true }).click();
